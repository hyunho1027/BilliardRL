    ...
    def __init__(self, name):
        with tf.variable_scope(name):
            ...
            if vis_mode:
                ...
                self.fc1 = tf.layers.dense(self.flat, 128, tf.nn.relu)
                self.fc2 = tf.layers.dense(self.fc1, 128, tf.nn.relu)
                self.fc3 = tf.layers.dense(self.fc2, 128, tf.nn.relu)

            else:
                self.fc1 = tf.layers.dense(self.s, 128, tf.nn.relu)
                self.fc2 = tf.layers.dense(self.fc1, 128, tf.nn.relu)
                self.fc3 = tf.layers.dense(self.fc2, 128, tf.nn.relu)
...
class PPOAgent:
    def __init__(self):
        ...
        self.v_loss = tf.reduce_mean((self.ret-self.model.v)**2)
        self.total_loss = self.pi_loss + self.v_loss
        self.train_model = tf.train.AdamOptimizer(lr).minimize(self.total_loss)

        self.approx_kl = tf.reduce_mean(self.logp_old - self.model.logp)
        self.approx_ent = tf.reduce_mean(-self.model.logp)
        ...
        for i in range(epoch):
            _, sub_v_loss, approx_kl, approx_ent = \
                 self.sess.run([self.train_model, self.v_loss, self.approx_kl, self.approx_ent],
                                feed_dict={self.model.s: s, self.model.s: s, self.model.a: a, self.ret: target, self.adv: adv, self.logp_old: logp_old})
            v_loss += sub_v_loss
            kl += approx_kl
            ent += approx_ent
    ...
    def get_action(self, s):
        a, v, logp_pi = self.sess.run([self.model.pi, self.model.v, self.model.logp_pi],
                                        feed_dict={self.model.s: s, self.model.s: s})
        return a, v, logp_pi
...
if __name__ == '__main__' :
    ...
    while True:
        ...
        for step in range(n_step):
            a, v_t, logp_t = agent.get_action([s])
